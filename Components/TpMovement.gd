...
func _ready():
	...
	CameraPivot.position.x = CameraTilt
	CameraPivot.spring_length = CameraDistance
...
func HandleCameraMovement(event):
	if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		...
		Geometry.rotate_y(deg_to_rad((event as InputEventMouseMotion).relative.x * MouseSensitivity))
		CameraPivot.rotate_x(deg_to_rad(-(event as InputEventMouseMotion).relative.y * MouseSensitivity))
		CameraPivot.rotation.x = deg_to_rad(clamp(rad_to_deg(CameraPivot.rotation.x), BottomAngleLimit, TopAngleLimit))
